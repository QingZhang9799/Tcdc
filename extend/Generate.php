    ...
    private function buildModel($path, $pathTemplate, $fileName, $tableName, $code, $data)
    {
        ...
        if (isset($this->data['form']) && $this->data['form']) {
            ...
            foreach ($this->data['form'] as $form) {
                if($form['type']=="picarr"){
                    $name = $this->parse_name($form['name'],1);
                    if($i == 0){
                        $autoArr .= '// 开启自动完成' . "\n"
                        . tab(1) . 'protected $auto = [\''.$form['name'].'\'';
                    }
                    else{
                        $autoArr .=',\''.$form['name'].'\'';
                    }
                   $autoFunction .= tab(1) . '// 修改器' . "\n"
                   . tab(1) . 'protected function set'.$name.'Attr($value)' . "\n"
                   . tab(1) . '{'. "\n"
                   . tab(2) . '$picarr = input(\''.$form['name'].'/a\');' . "\n"
                   . tab(2) . 'if(is_array($picarr))' . "\n"
                   . tab(2) . '{'. "\n"
                   . tab(3) . '$info = input(\'info/a\');' . "\n"
                   . tab(3) . '$show = input(\'show/a\');' . "\n"
                   . tab(3) . 'foreach($picarr as $key=>$v){'. "\n"
                   . tab(4) . '$row[$key][\'img\'] = $v;' . "\n"
                   . tab(4) . '$row[$key][\'info\'] = $info[$key];' . "\n"
                   . tab(4) . '$row[$key][\'show\'] = $show[$key];' . "\n"
                   . tab(3) . '}' . "\n"
                   . tab(3) . 'return serialize($row); ' . "\n"
                   . tab(2) . '}else{' . "\n"
                   . tab(3) . 'return \'\';' . "\n"
                   . tab(2) . '}' . "\n"
                   . tab(1) . '}'. "\n"
                   . tab(1) . '// 获取器' . "\n"
                   . tab(1) . 'protected function get'.$name.'Attr($value)' . "\n"
                   . tab(1) . '{'. "\n"
                   . tab(2) . '$data = empty($value)?[]:unserialize($value);' . "\n"
                   . tab(2) . 'return $data;' . "\n"
                   . tab(1) . '}' . "\n";
        ...
        $auto = $autoTimestamp . $autoArr . $autoFunction ;

        return file_put_contents($file, str_replace(
                ["[MODULE]", "[TITLE]", "[NAME]", "[NAMESPACE]", "[TABLE]", "[AUTO_TIMESTAMP]"],
                [$module, $this->data['title'], $this->name, $this->namespaceSuffix, $tableName, $auto],
                $template
            )
        );
    }
                ...
                if (!in_array($form['name'], ['id', 'isdelete', 'orderid', 'create_time', 'update_time'])) {
                    ...
                    switch ($form['type']) {
                        ...
                        case "picurl":
                            ...
                            $setSelected[] = tab(1) . 'fileinputOne("fileone' . $key . '","picurl' . $key . '");';
                            $createField .= tab(4) . '<input id="fileone' . $key . '" type="file" class="file-loading">'. "\n"
                                . tab(4) . '<input type="hidden" id="picurl' . $key . '" placeholder="' . $form['title'] . '" name="' . $form['name'] . '" '
                                . 'value="' . '{$vo.' . $form['name'] . ' ?? \'' . $form['default'] . '\'}' . '" '
                                . '>' . "\n";
                            break;
                        case "picarr":
                            $setFileinput = '{block name="fileinput"}{__block__}{/block}';
                            $setSelected[] = tab(1) . 'fileinputMore("filepic' . $key . '","picimg' . $key . '[]","' . $form['name'] . '[]");';
                            $createField .= tab(4) . '<input id="filepic' . $key . '" type="file" multiple>'. "\n"
                                 . tab(4) . '{present name="vo"}'. "\n"
                                 . tab(6) . '{volist name="vo.' . $form['name'] . '" id="vop"}'. "\n"
                                 . tab(8) . '<input type="hidden" value="{$vop.img}" name="picimg' . $key . '[]" flag="{$vop.info}" show="{$vop.show}" >'. "\n"
                                 . tab(6) . '{/volist}'. "\n"
                                 . tab(4) . '{/present}'. "\n";
                            break;
                        case "date":
