...
class Estimateprice extends Base
{
    public function index()
    {
        $params = [
            ...
            "partnerCarTypeIdList" => input('?partnerCarTypeIdList') ? input('partnerCarTypeIdList') : null,
            "cityId" => input('?cityId') ? input('cityId') : null,
            "departureTime" => input('?departureTime') ? input('departureTime') : null,
        ];
//        $file = fopen('./estimate.txt', 'a+');
//        fwrite($file, "-------------------预估价参数11:--------------------" .json_encode($params). "\r\n");
