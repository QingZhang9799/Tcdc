...
class Useriospay extends Controller{
    //充值
	public function zfbsqpost(RSACrypt $crypt){
        ...
        $order_id = Db::name('recharge_order')->where(['ordernum'=>$datas['ordernum']])->value('id') ;
        $passback_params = $datas['passback_params'].",".$order_id;

//        $aop = new \AopClient();
        $aop = new \AopClient();
        $aop->gatewayUrl = "https://openapi.alipay.com/gateway.do";
        $aop->appId = "2021001174687362";
        ...
        $bizcontent = "{\"body\":\"$title\","
            ...
            . "}";
        //TODO 上线时需要修改为正式地址
        $request->setNotifyUrl("https://php.51jjcx.com/user/useriospay/alipay");
...
//        return $crypt->response(['code'=>200,'msg'=>'成功','data'=>$response]);
    ...
    public function alipay(Request $request)
    {
        ...
        fwrite($file, "--------------------------进来了-------------------"."\r\n");
        fwrite($file, "ios支付宝支付信息：" . json_encode($data) . "\r\n");
...
//            $alipaySevice->writeLog('支付类型：' . $status);
...
//            }
  	...
  	public function alituiorder($ordernum){
    	include_once  "extend/Alipay/wappay/service/AlipayTradeService.php" ;
    	include_once  "extend/Alipay/aop/AopClient.php";
    	include_once  "extend/Alipay/aop/request/AlipayTradeRefundRequest.php" ;
        $orderinfo = Db::name('goodsorder')->where('ordernum',$ordernum)->find();
        $refund_amount = $orderinfo['totalprice'];
        $aop = new \AopClient ();
        $aop->gatewayUrl = 'https://openapi.alipay.com/gateway.do';
        $aop->appId = '2019042264281149';
        $aop->rsaPrivateKey = '';
      	$aop->alipayrsaPublicKey='';  
      	$aop->apiVersion = '1.0';
        $aop->signType = 'RSA2';
