...

{
    public function getSign($params) {
        ...
        $stringSignTemp = $stringA."&key=".'zihai906zihai906zihai906zihai906';        //拼接key
...
/*
    ...
    public function wxpaypost(RSACrypt $crypt){
        ...
        $data['appid']            = 'wxb1ff806d3e2480a2'; //appid
        $data['mch_id']           = '1522837421'; //商户ID
        $data['nonce_str']        = $nonce_str; //随机字符串 这个随便一个字符串算法就可以，我是使用的UUID
        $data['body']             = '众行外卖'; // 商品描述
        $data['out_trade_no']     = $datas['ordernum'];    //商户订单号,不能重复
        $data['total_fee']        = $datas['total_fee'] * 100; //金额
        $data['spbill_create_ip'] = $_SERVER['SERVER_ADDR'];   //ip地址
        $data['notify_url']       = 'http://app174.app.longcai.net/user_api/weixin/notifyurl';
        ...
        if($data){
            ...
            if($re['return_code'] != 'SUCCESS'){
                ...
                $arr =array(
                    ...
                    'appid'     => 'wxb1ff806d3e2480a2',
                    'partnerid' => '1522837421', // 商户ID
                    'package'   => 'Sign=WXPay',
                    'noncestr'  => $nonce_str,
                    'timestamp' =>time(),
                );
