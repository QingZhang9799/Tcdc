...
//        $filepath = 'D:/phpStudy/PHPTutorial/WWW/tcdc/public/static/home/'.'bg_img.jpg';//图片地址自己写
        $path = ROOT_PATH.'public/static/bjnews/images/20210401161838.jpg' ;
         $file_info = array(
            'filename'=>ROOT_PATH.'public/static/bjnews/images/20210401161838.jpg',
             'content-type'=>'image/png',
             'filelength'=>'90011',
         );
//        $rs = $this->add_material($file_info,$this->getAccessToken());
        $rs = $this->add_materials($path,$this->getAccessToken());
        var_dump($rs);
//        $thumb_res = json_decode($rs,true);
//        var_dump($thumb_res);
...
//        curl_setopt ( $ch1, CURLOPT_POSTFIELDS, $data );
        curl_setopt($ch1, CURLOPT_POSTFIELDS, $data);// 处理请求数据

        $result = curl_exec ( $ch1 );
        curl_close ( $ch1 );
        return $result;
    }
    function add_materials($real_path,$token){
        $url="https://api.weixin.qq.com/cgi-bin/material/add_material?access_token={$token}&type=image";
        $curl = curl_init();
//        $real_path="{$_SERVER['DOCUMENT_ROOT']}{$file_info['filename']}";
